...
export function useProjects(page: number = 0, limit: number = 10, search: string = "", sortBy: string = "createdAt", sortDir: string = "desc", questionType: string = "") {
  return useQuery({
    queryKey: ["projects", page, limit, search, sortBy, sortDir, questionType],
    queryFn: () => getProjects(page, limit, search, sortBy, sortDir, questionType),
  });
}

import { useInfiniteQuery } from "@tanstack/react-query";

export function useInfiniteProjects(limit: number = 10, search: string = "", sortBy: string = "createdAt", sortDir: string = "desc", questionType: string = "") {
  return useInfiniteQuery({
    queryKey: ["projects", "infinite", limit, search, sortBy, sortDir, questionType],
    queryFn: ({ pageParam = 0 }) => getProjects(pageParam, limit, search, sortBy, sortDir, questionType),
    initialPageParam: 0,
