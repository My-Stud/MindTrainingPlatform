...
const getQuestions = async (req, res) => {
  const { search = '', difficulty = '', category = '', folderId } = req.query;
...
const addQuestion = async (req, res) => {
  ...
  const { field1, field2, field3, optionA, optionB, optionC, optionD, correctAnswer, hint, difficulty, category, folderId } = req.body;
...
const uploadQuestions = async (req, res) => {
  ...
  const { mapping, folderId } = req.body; // e.g. column index mapping
  let rows = [];
  ...
  if (rows.length === 0) {
    ...
    let colMap;
    if (isMcq) {
      colMap = { field1: 0, optionA: 1, optionB: 2, optionC: 3, optionD: 4, correctAnswer: 5, hint: 6 };
    } else {
    ...
    if (mapping) {
      if (isMcq) {
        ...
        const hasQuestionHeader = headerRow.some(c => c.includes('question') || c.includes('field1'));
        console.log("HAS QUESTION HEADER: ", hasQuestionHeader);

      if (hasQuestionHeader) {
        if (isMcq) {
          colMap = { field1: -1, optionA: -1, optionB: -1, optionC: -1, optionD: -1, correctAnswer: -1, hint: -1 };
          headerRow.forEach((c, idx) => {
            if (c.includes('question')) colMap.field1 = idx;
            else if (c.includes('answer') || c.includes('correct')) colMap.correctAnswer = idx;
            else if (c.includes('hint') || c.includes('clue')) colMap.hint = idx;
            else if (c.includes('option1') || c === 'a' || c === 'option a') colMap.optionA = idx;
            else if (c.includes('option2') || c === 'b' || c === 'option b') colMap.optionB = idx;
            else if (c.includes('option3') || c === 'c' || c === 'option c') colMap.optionC = idx;
            else if (c.includes('option4') || c === 'd' || c === 'option d') colMap.optionD = idx;
          });
          // Fallback for unmapped
          if (colMap.field1 === -1) colMap.field1 = 0;
          ...
          if (colMap.hint === -1) colMap.hint = 6;
        } else {
          colMap = { field1: -1, field2: -1, field3: -1 };
          headerRow.forEach((c, idx) => {
            if (c.includes('question') || c.includes('word')) colMap.field1 = idx;
            else if (c.includes('answer') || c.includes('definition')) colMap.field2 = idx;
            else if (c.includes('hint') || c.includes('clue')) colMap.field3 = idx;
          });
          if (colMap.field1 === -1) colMap.field1 = 0;
          if (colMap.field2 === -1) colMap.field2 = 1;
          if (colMap.field3 === -1) colMap.field3 = 2;
        }
        rows.shift(); // Remove header row since we processed it!
      }
    }
  ...
  if (!targetFolderId) {
    const folder = await prisma.folder.create({
      data: { name: req.file.originalname, projectId: project.id }
    });
    targetFolderId = folder.id;
  }

  const docsWithFolder = docs.map(d => ({ ...d, folderId: targetFolderId }));
  const result = await prisma.question.createMany({ data: docsWithFolder });
  res.status(201).json({
...
const parseDocx = async (buffer, isMcq = false) => {
  // First, check for HTML tables which represent Word tables
  ...
  while ((trMatch = trRegex.exec(html)) !== null) {
    ...
    while ((tdMatch = tdRegex.exec(trContent)) !== null) {
      // Extract text from HTML by replacing <p> with spaces and removing other tags
  ...
  // If we found a valid table (more than just a header row), assume it's a tabular DOCX
  if (tableRows.length > 1) {
    // Check if the first row is a header row (e.g. contains 'question', 'answer')
    const firstRowText = tableRows[0].join(' ').toLowerCase();
    // Removed shift here so uploadQuestions can auto-detect colMap
    return tableRows;
  }
  ...
  if (isMcq) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (ansMatch && currentQ) {
        ...
        const hintSplit = ansText.match(/^(.*?)\s*(?:Hint|Clue)[\:\.\-\s]+(.*)/i);
        if (hintSplit) {
          ansText = hintSplit[1].trim();
          currentQ.hint = hintSplit[2].trim();
        }
        currentQ.correctAnswer = ansText;
        ...
        // Parse possible inline options e.g. "A) Japan B. China C. South Korea D. Mongolia" (even without spaces)
        const allOptsRegex = /(?:^|\s|(?<=[a-z]))(?:Option\s+|Opt\s+)?([A-D])[\)\.\:\-]\s*((?:(?!(?:^|\s|(?<=[a-z]))(?:Option\s+|Opt\s+)?[A-D][\)\.\:\-]\s).)+)/gi;
        const matches = [...line.matchAll(allOptsRegex)];
        
        if (matches.length > 1) {
          matches.forEach(m => {
            const letter = m[1].toUpperCase();
            const text = m[2].trim();
            if (letter === 'A') currentQ.optionA = text;
            else if (letter === 'B') currentQ.optionB = text;
            else if (letter === 'C') currentQ.optionC = text;
            else if (letter === 'D') currentQ.optionD = text;
          });
        } else {
