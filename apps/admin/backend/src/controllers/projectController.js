...
const projectToResponse = (p) => ({
  id: p.id.toString(),
  quizFolderId: p.activeFolderId ? p.activeFolderId.toString() : null,
  _id: p.id,
  name: p.name,
  slug: p.slug,
  ...
  numberOfQuestionsInQuiz: p.questionsPerQuiz,
...
const getProjects = async (req, res) => {
  ...
  const UPLOAD_QUESTION_PROJECTS = [
    'Country Shooter', 'Tactical Sniper', 'Tanker', 'City Runner', 'Platformer',
    'Vocabullseye', 'State Capital Shooter', 'Cactiquiz', 'Trivia Smash',
    'Institute Orbit', 'Mystery Pointer', 'Helicopter Trivia', 'Vocabkicker'
  ];

  const where = { AND: [] };
  if (search) {
    where.AND.push({ name: { contains: search } });
  }
  if (questionType === 'upload') {
    where.AND.push({ name: { in: UPLOAD_QUESTION_PROJECTS } });
  } else if (questionType === 'no-question') {
    where.AND.push({ name: { notIn: UPLOAD_QUESTION_PROJECTS } });
  }
...
const createProject = async (req, res) => {
  const { name, fieldLabels, mainQuestionField, questionsPerQuiz, numberOfQuestionsInQuiz, projectType, allowedOrigins } = req.body;
  ...
  const project = await prisma.project.create({
    data: {
      ...
      questionsPerQuiz: numberOfQuestionsInQuiz || questionsPerQuiz || 15,
...
const updateProject = async (req, res) => {
  ...
  const { name, fieldLabels, mainQuestionField, questionsPerQuiz, numberOfQuestionsInQuiz, projectType, allowedOrigins, activeFolderId, quizFolderId } = req.body;
  const updateData = {};

  if (quizFolderId !== undefined) {
    updateData.activeFolderId = quizFolderId === null ? null : Number(quizFolderId);
  }
  if (activeFolderId !== undefined) {
    updateData.activeFolderId = activeFolderId === null ? null : Number(activeFolderId);
  }
  ...
  if (numberOfQuestionsInQuiz !== undefined) updateData.questionsPerQuiz = Number(numberOfQuestionsInQuiz);
    if (questionsPerQuiz !== undefined) updateData.questionsPerQuiz = Number(questionsPerQuiz);
